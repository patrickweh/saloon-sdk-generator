    ...
    protected function generateDtoClass($className, Schema $schema)
    {
        ...
        foreach ($properties as $propertyName => $propertySpec) {
            ...
            if (empty($propertyName)) {
                echo "WARNING: Skipping empty property name in DTO: $dtoName\n";

                continue;
            }

            // Handle numeric property names by prefixing them
            if (is_numeric($propertyName)) {
                $name = 'property_' . $propertyName;
            } else {
                $name = NameHelper::safeVariableName($propertyName);

                // Additional check after sanitization
                if (empty($name)) {
                    // If sanitization resulted in empty name, prefix with 'property_'
                    $name = 'property_' . preg_replace('/[^a-zA-Z0-9_]/', '_', $propertyName);
                }
            }
